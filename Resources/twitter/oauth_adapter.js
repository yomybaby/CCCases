...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod)
 {
	...
	var windowDestroyCallback = null;
    ...
    var destroyAuthorizeUI = function(dontExecDestroyCallBack)
    {
        ...
        try
        {
            ...
            view = null;
            window = null;
			if(windowDestroyCallback) setTimeout(windowDestroyCallback, 100);
        }
        catch(ex)
        {
    ...
    // webview의 로딩 인디케이터 
    wbActInd = Titanium.UI.createActivityIndicator({
		width:30,
		height:30,
		top:227
	});
	wbActInd.style = Titanium.UI.iPhone.ActivityIndicatorStyle.DARK;
    ...
    var authorizeUICallback = function(e)
    {
		...
		var arr = reg.exec(e.source.html);
    ...
    this.showAuthorizeUI = function(pUrl, pReceivePinCallback,pDestroyCallBack)
    {
        receivePinCallback = pReceivePinCallback;
		windowDestroyCallback = pDestroyCallBack;
    ...
    var send = function(pUrl, pParameters, pTitle, pSuccessMessage, pErrorMessage)
    {
        ...
        if (client.status == 200) {
			if(typeof(pSuccessMessage)=='function'){
				setTimeout(pSuccessMessage, 100);
			} else {
			...
			if (typeof(pErrorMessage) == 'function') {
				setTimeout(pErrorMessage, 100);
			}
			else {
				Ti.UI.createAlertDialog({
					title: pTitle,
